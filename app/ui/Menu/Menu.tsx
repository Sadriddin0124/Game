...
const Menu: React.FC = () => {
  const links: Array<{ id: number, path: string, title: string }> = [
    { id: 1, path: "/continue", title: "Continue" },
    { id: 2, path: "/dashboard/level-one", title: "New Game" },
    { id: 3, path: "/settings", title: "Settings" },
    { id: 4, path: "/quit", title: "Quit" },
  ];
        ...
        <ul className='flex flex-col w-[100%] gap-[10px] items-center'>
          {
            links?.map((item, index) => {
              return (
                <li onMouseEnter={handlePlay} key={index} className='bg-[#ffffff3e] w-[100%] h-[100%] py-[10px] px-[20px] rounded-md cursor-pointer duration-75 text-center ease-linear hover:border-[#ffffff57] backdrop-blur-md hover:bg-[#ffffff57] border-transparent border'>
